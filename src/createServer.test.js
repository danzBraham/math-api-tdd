...
describe('A HTTP Server', () => {
  ...
  describe('When GET /triangle/area', () => {
    it('should respond with a status code of 200 and the payload value of the calculation result of the area of a triangle formula', async () => {
      // Arrange
      const figureCalculator = new FigureCalculator(MathBasic);
      const height = 30;
      const base = 50;
      const expectedAnswer = figureCalculator.calculateTriangleArea(height, base);
      ...
      expect(response.statusCode).toEqual(200);
      expect(responseJson.value).toEqual(expectedAnswer);
      expect(spyTriangleArea).toHaveBeenCalledWith(height, base);
    });
  });
});
